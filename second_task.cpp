...
void BlockCopying(const string &src, const string &dst)
{
    ...
    const DWORD overlapped_count = src_size % cluster_size == 0 ? src_size / cluster_size : src_size / cluster_size + 1;
    ...
    // cout << s << endl;
    // if (write_complete != overlapped_count)
    // {
    //     SleepEx(INFINITE, TRUE);
    // }
    ...
    BOOL t =  SetFilePointer(dst_handle, src_size_quad.LowPart, &src_size_quad.HighPart, 0);
       if (!t){
        cout << "SetFilePointer executed wrong" << endl;
    }
    BOOL t =  SetEndOfFile(dst_handle);
       if (!t){
        cout << "SetEndOfFile executed wrong" << endl;
    }
...
{
    std::ofstream ofs;
    ofs.open("C:\\Users\\PickNick\\Desktop\\dataset\\vasya.txt", std::ofstream::out | std::ofstream::trunc);
    ofs.close();
    BlockCopying(src, "C:\\Users\\PickNick\\Desktop\\dataset\\vasya.txt");
}
int main()
{
    cluster_size = 8192;
    overlapped_operations_count = 8;
    // vector<string> v = {"C:\\Users\\PickNick\\Desktop\\dataset\\1000", "C:\\Users\\PickNick\\Desktop\\dataset\\100000", "C:\\Users\\PickNick\\Desktop\\dataset\\256_000"};
    // vector<DWORD> v_int = {8, 12, 16, 32};
    ...
    //     for (cluster_size = 8192; cluster_size < 32769; cluster_size += 4096)
    //     {
    //         for (const auto &a : v)
    //         {
    ...
    ofs.open("C:\\Users\\PickNick\\Desktop\\dataset\\vasya.txt", std::ofstream::out | std::ofstream::trunc);
    ofs.close();
    BlockCopying("C:\\Users\\PickNick\\Desktop\\dataset\\.txt", "C:\\Users\\PickNick\\Desktop\\dataset\\vasya.txt");
    //    writeFileWrapper();
    //    readFileWrapper();
}
